...
import { getTechnologyById, updateTechnology, deleteTechnology } from '../utils/technologiesStorage';
...
function TechnologyDetail() {
  ...
  const updateStatus = (newStatus) => {
    if (technology) {
      const updated = updateTechnology(technology.id, { status: newStatus });
  ...
  const handleDelete = () => {
    if (window.confirm('Вы уверены, что хотите удалить эту технологию?')) {
      if (technology && deleteTechnology(technology.id)) {
